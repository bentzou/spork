...
p1    feat/search-index-sync       branch 21h   Build search index syncer
p3    fix/image-cache-expiry       branch 5d    Fix image cache expiry bug
p4    main                         local  12d   Spike: sqlite cache backend   ← stale: AGE turns red after 7d
p5    feat/storage-interfaces      branch 1d    Abstract the storage backend
p2    main                         ready
p6    main                         ready
```
...
- **SESSION** — title of that same most-recent session: the `aiTitle`
  ...
  table as "*p3* is mid-flight on *fix/image* — it's the *cache expiry*
