...
# Derives workspace paths from this file's location and loads .spork/config.
#
# Exports: SPORK_DIR BASE_DIR RUNTIME_DIR MIRROR_DIR LOG_FILE LOCK_DIR
#          ORIGIN_URL TRUNK_BRANCH
...
if [[ ! -f "$SPORK_DIR/config" ]]; then
    echo "Missing $SPORK_DIR/config — set ORIGIN_URL and TRUNK_BRANCH there." >&2
    exit 1
fi
# shellcheck source=/dev/null
. "$SPORK_DIR/config"

: "${ORIGIN_URL:?ORIGIN_URL must be set in $SPORK_DIR/config}"
: "${TRUNK_BRANCH:?TRUNK_BRANCH must be set in $SPORK_DIR/config}"
# shellcheck disable=SC2034
: "${CLONE_PREFIX:=p}"
